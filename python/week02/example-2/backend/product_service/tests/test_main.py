# product_service/tests/test_main.py
...
# Import app, engine, get_db, and SessionLocal from your main application's modules.
# Ensure these imports are correct based on your project structure.
# Assuming 'tests' is a sibling directory to 'app' within 'product_service'.
from app.main import app
from app.db import get_db, engine, SessionLocal # Import engine here as well
from app.models import Product, Base # Import Base to create tables on startup
...
def setup_database_for_tests():
    """
    Ensures the main application's database is reachable and its tables are created
    before any tests run. This runs once per test session.
    
    CRITICAL FIX: Explicitly drops all tables before creating them to ensure a truly
    clean slate at the start of the test session, clearing any data from previous runs.
    """
    max_retries = 10
    retry_delay_seconds = 3
    for i in range(max_retries):
        try:
            logging.info(f"Attempting to connect to main PostgreSQL for test setup (attempt {i+1}/{max_retries})...")
            ...
            logging.info("Successfully dropped all tables in main PostgreSQL for test setup.")

            # Then create all tables required by the application
            Base.metadata.create_all(bind=engine)
            logging.info("Successfully created all tables in main PostgreSQL for test setup.")
            break
        except OperationalError as e:
            logging.warning(f"Test setup DB connection failed: {e}. Retrying in {retry_delay_seconds} seconds...")
            time.sleep(retry_delay_seconds)
            if i == max_retries - 1:
                pytest.fail(f"Could not connect to PostgreSQL for test setup after {max_retries} attempts: {e}")
        except Exception as e:
            pytest.fail(f"An unexpected error occurred during test DB setup: {e}", pytrace=True)
    
    yield # Yield control to the tests

    # Optional: If you want to explicitly drop tables after all tests in the session, uncomment:
    # logging.info("Dropping tables from main PostgreSQL after test session.")
    # Base.metadata.drop_all(bind=engine)


@pytest.fixture(scope="function") # 'function' scope ensures a fresh DB state for each test
...
def client():
    ...
    """
    # The `app.dependency_overrides[get_db]` is set by `db_session_for_test` fixture
...
def test_create_product_missing_required_field(client: TestClient): # Removed db_session_for_test: Session
    """
    ...
    assert any("Field required" in err["msg"] and err["loc"][1] == "name" for err in response.json()["detail"])

def test_list_products_empty(client: TestClient): # Removed db_session_for_test: Session
    """
    ...
    """
    # The `setup_database_for_tests` fixture (session scope) ensures the database is wiped
    # at the start of the entire test run. The `db_session_for_test` (function scope)
    # ensures each individual test function operates on an isolated transaction.
    # Therefore, if no products are created *within this specific test's transaction*,
    # the list should be empty.
    response = client.get("/products/")
    assert response.status_code == 200
    assert response.json() == []

def test_list_products_with_data(client: TestClient, db_session_for_test: Session): # db_session_for_test added
    """
...
def test_list_products_pagination(client: TestClient, db_session_for_test: Session): # db_session_for_test added
    """
...
def test_list_products_search(client: TestClient, db_session_for_test: Session): # db_session_for_test added
    """
    Tests searching products by name or description.
    """
    # Create specific products via API for search
    client.post("/products/", json={"name": "Apple Laptop", "description": "Powerful machine", "price": 1000, "stock_quantity": 10})
    client.post("/products/", json={"name": "Banana Phone", "description": "Fruit-themed device", "price": 500, "stock_quantity": 20})
    client.post("/products/", json={"name": "Orange Juice", "description": "Freshly squeezed", "price": 5, "stock_quantity": 50})
...
def test_get_product_success(client: TestClient, db_session_for_test: Session): # db_session_for_test added
    """
    Tests successful retrieval of a product by ID.
    """
    # Create a product via API for this test
    product_data = {"name": "Get Product Test", "description": "Temp desc", "price": 15.00, "stock_quantity": 20}
...
def test_get_product_not_found(client: TestClient): # Removed db_session_for_test: Session, as this test does not need direct DB interaction
    """
...
def test_update_product_full(client: TestClient, db_session_for_test: Session): # db_session_for_test added
    """
    Tests full update of a product's details.
    """
    # Create a product for update test
    product_data = {"name": "Product To Update", "description": "Original description", "price": 20.00, "stock_quantity": 30}
    create_response = client.post("/products/", json=product_data)
    product_id = create_response.json()["product_id"]
    ...
    # `updated_at` comparison can be tricky with fixtures/transactions and should be handled with care
    # For simplicity, we'll omit explicit timestamp comparison here unless it's critical.

def test_update_product_partial(client: TestClient, db_session_for_test: Session): # db_session_for_test added
    """
    Tests partial update of a product (e.g., only name).
    """
    # Create a product for partial update test
    product_data = {"name": "Original Name", "description": "Original Desc", "price": 10.00, "stock_quantity": 10}
    create_response = client.post("/products/", json=product_data)
    product_id = create_response.json()["product_id"]
...
def test_update_product_not_found(client: TestClient): # Removed db_session_for_test: Session
    """
...
def test_delete_product_success(client: TestClient): # Removed db_session_for_test: Session
    """
...
def test_delete_product_not_found(client: TestClient): # Removed db_session_for_test: Session
    """
