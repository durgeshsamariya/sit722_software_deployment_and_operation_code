...
async def create_order(order_data: OrderCreate, db: Session = Depends(get_db)):
    ...
    for item_data in order_data.items:
        ...
        item_total = Decimal(str(item_data.quantity)) * Decimal(
            str(item_data.price_at_purchase)
        )
    ...
    try:
        ...
        db.refresh(
            db_order
        )  # Refresh to get auto-generated IDs and timestamps for order and items

        logger.info(
            f"Order Service: Order {db_order.order_id} created successfully for user {db_order.user_id}."
        )
        return db_order
    except Exception as e:
        db.rollback()
