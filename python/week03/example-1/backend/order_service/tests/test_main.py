...
from decimal import Decimal # For precise comparisons of monetary values

# Import app, engine, get_db, SessionLocal, and Base from your main application's modules.
# Ensure these imports are correct based on your project structure relative to this test file.
...
def test_create_order_invalid_quantity(client: TestClient, db_session_for_test: Session):
    """
    ...
    invalid_order_data = {
        "user_id": 2,
        "items": [
            {"product_id": 103, "quantity": 0, "price_at_purchase": 5.00} # Invalid quantity
        ]
    }
    response = client.post("/orders/", json=invalid_order_data)
    assert response.status_code == 422
    assert "detail" in response.json()
    assert any("value must be greater than or equal to 1" in err["msg"] for err in response.json()["detail"])
...
def test_create_order_missing_items(client: TestClient, db_session_for_test: Session):
    ...
    assert response.status_code == 422
    assert "detail" in response.json()
    assert any("Field required" in err["msg"] and err["loc"][1] == "items" for err in response.json()["detail"])
...
def test_list_orders_filter_by_user_id(client: TestClient, db_session_for_test: Session):
    """
