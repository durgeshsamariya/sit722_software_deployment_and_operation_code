...
Now includes synchronous communication with the Product Service to deduct stock
...
async def create_order(
    ...
    async with httpx.AsyncClient() as client:
        for item in order.items:
            ...
            logger.info(f"Order Service: Attempting to deduct stock for product {product_id} (qty: {quantity}) via Product Service at {deduct_stock_url}")
            ...
            try:
                ...
                logger.info(f"Order Service: Stock deduction successful for product {product_id}.")
                ...
                logger.error(f"Order Service: Stock deduction failed for product {product_id}: {error_detail}. Status: {e.response.status_code}")
                ...
                logger.critical(f"Order Service: Network error communicating with Product Service for product {product_id}: {e}")
                await _rollback_stock_deductions(client, successfully_deducted_items)
                raise HTTPException(
                ...
                logger.error(f"Order Service: An unexpected error occurred during stock deduction for product {product_id}: {e}", exc_info=True)
                await _rollback_stock_deductions(client, successfully_deducted_items)
                raise HTTPException(
    ...
    try:
        ...
        logger.info(f"Order Service: Order {db_order.order_id} created and confirmed successfully for user {db_order.user_id}.")
        return db_order
    except Exception as e:
        db.rollback()
        logger.error(f"Order Service: Error creating order after successful stock deductions: {e}", exc_info=True)
...
async def _rollback_stock_deductions(client: httpx.AsyncClient, items: List[OrderItem]):
    ...
    logger.warning("Order Service: Attempting to rollback stock deductions due to order creation failure.")
    for item in items:
        product_id = item.product_id
        quantity = item.quantity
        add_stock_url = f"{PRODUCT_SERVICE_URL}/products/{product_id}/deduct-stock" # Assuming -ve quantity adds stock
        
        # NOTE: A proper rollback would require a separate "add-stock" or "increase-stock" endpoint
        # or accepting negative quantities with explicit business logic for refunds/returns.
        # For simplicity, we are reusing deduct-stock with negative quantity,
        # which implies the deduct-stock endpoint in Product Service must handle this.
        # However, for a simple deduct endpoint, it's better to have an explicit 'add-stock' endpoint.
        # Let's assume Product Service's deduct-stock ONLY deducts positive values.
        # Thus, for a real rollback, the Product Service should expose an "add-stock" endpoint.
        # For now, we will just log and acknowledge this limitation.
        
        # Simulating a rollback by calling a hypothetical 'add-stock' endpoint.
        # Since we don't have an explicit 'add-stock' endpoint, we will log a warning
        # that manual intervention might be required if stock cannot be recovered.
        
        logger.warning(f"Order Service: Cannot automatically rollback stock for product {product_id} quantity {quantity}. Manual stock adjustment may be required in Product Service.")
        
        # A more robust solution for synchronous two-phase commit/rollback is complex
        # and often leads to distributed transactions or sagas. For simple sync,
        # alerting for manual rollback is often the fallback.
        # Example if there was an actual add-stock endpoint:
        # try:
        #     rollback_response = await client.patch(
        #         f"{PRODUCT_SERVICE_URL}/products/{product_id}/add-stock",
        #         json={"quantity_to_add": quantity}
        #     )
        #     rollback_response.raise_for_status()
        #     logger.info(f"Order Service: Stock rollback successful for product {product_id}.")
        # except Exception as rollback_e:
        #     logger.error(f"Order Service: FAILED to rollback stock for product {product_id}: {rollback_e}")
